...
const DataDisplayHover = ({ data }) => {
  ...
  const { count, healthy, unhealthy, workingSituations } = data;
  ...
  return (
    <>
      ...
      {workingSituations && workingSituations.buckets.length > 0 && (
        <Repeat large>
          <H3>Arbetssituation:</H3>
          <DataBoxGrid>
            <RepeatList healthList={workingSituations.buckets} count={count} />
          </DataBoxGrid>
        </Repeat>
      )}
    </>
